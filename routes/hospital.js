...
app.get('/:id', (req, res) => {
    var id = req.params.id;
    Hospital.findById(id).populate('usuario', 'nombre img email') // con esto obtengo el objeto usuario entero asociado a este find y solo muestro id, nombre y email
        .exec((err, hospital) => {
            if (err) {
                return res.status(500).json({
                    ok: false,
